    ...
    if request.method == 'POST':
        ...
        if file and file.filename:
            ...
            if extension not in ext2conttype:
                return "Extension '%s' non gérée, les extensions gérées sont %s" % (extension,
            ...
            try:
                ...
                print("BadRequestError catched while accessing request.form['resize']")
            if resize:
                image = resize_1024p(image)
            buff_img = BytesIO()
            image.seek(0)
            image.save(buff_img, ext2conttype2[extension])
            if buff_img.tell() > 5000000:
                return 'Le fichier est trop large, limite max : 5Mo'
            ...
            print("new file uploaded: %s" % get_url)
...
'''
@app.route('/media/get/resize/<img_id>', methods=['GET'])
def get_image_thumbnail(img_id):
	...
	print("Numbers is %s %s"%(desired_width, desired_height))
	...
	buffer_image.seek(0)
	return send_file(buffer_image, mimetype=content_type.decode('ascii'))
'''
